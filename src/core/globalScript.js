  ...
  class cache_temp{
    ...
    log(message, ...args) {
      if (this.debug) {
          console.log(`[Cache_Temp] ${message}`, ...args);
      }
    }
  ...
  class CreateElement {
    static projectTemplate(projectName, author, version) {
      return `<div project-id="${projectName}" class="project-preview-parent visible max-w-[calc(100%-1rem)] p-6 mb-4 mx-4 bg-gray-50 border border-gray-200 rounded-lg shadow hover:bg-gray-200">

        <div class="flex items-center justify-between w-full">
          <!-- Left Content -->
          <div>
            <h5 class="mb-2 text-sm font-bold tracking-tight text-gray-900">${projectName}</h5>
            <p class="text-xs font-normal text-gray-700">Author: ${author}</p>
            <p class="text-xs font-normal text-gray-700">Version: ${version}</p>
          </div>

          <!-- Right Button -->
          <div>
            <button id="duplicateProject" projectName="${projectName}" class="text-white bg-gray-500 hover:bg-gray-600 font-medium rounded-full text-sm p-1">
