...
export class PrimitiveColorModel extends BaseModel {
  constructor() {
  ...
  async create({ projectId, primitiveName, primitiveValue, orderIndex } = {}) {
    ...
    try {
      ...
      console.log(`[CREATE] PrimitiveColor added with ID ${primitiveId}`);
      ...
      console.error("[ERROR] Failed to add PrimitiveColor");
      throw error;
    }
  }
  ...
  async update({ id, updatedFields } = {}) {
    ...
    try {
      ...
      if (updatedCount === 0) {
        console.warn(`[WARN] No primitive color found with ID: ${id}`);
      } else {
        cacheManager.primitives.update(id, updatedFields);
        console.log(`[SUCCESS] Updated primitive color with ID: ${id}`);
      }

      return updatedCount;
    } catch (error) {
      console.error(`[ERROR] Failed to update primitive color with ID: ${id}`);
      throw error;
    }
  }
  ...
  async delete({ id }) {
    ...
    try {
      ...
      console.log(`[DELETE] PrimitiveColor with ID ${id} deleted`);
      cacheManager.primitives.delete(id);
      return true;
    } catch (error) {
      console.error("[ERROR] Failed to delete PrimitiveColor");
      throw error;
    }
  }
}
